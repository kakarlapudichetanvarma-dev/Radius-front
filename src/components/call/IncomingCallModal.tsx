...
export default function IncomingCallModal({ onAccepted }: Props) {
  ...
  const handleAccept = async () => {
    ...
    try {
      if (!callState.offerSdp || !callState.sessionId) {
        throw new Error('Missing offer or session ID');
      }
  ...
  const handleDecline = () => {
    if (callState.sessionId) sendCallDecline(callState.sessionId);
    dispatch(rejectCall());
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
      <div className="bg-zinc-900 rounded-2xl p-8 text-center shadow-2xl w-80">
        ...
        <p className="text-gray-400 text-sm mb-1">Incoming {isVideo ? 'Video' : 'Audio'} Call</p>
        <h2 className="text-white text-xl font-bold mb-6">{callerName}</h2>

        <div className="flex items-center justify-center gap-6">
          <button onClick={handleDecline} className="w-14 h-14 rounded-full bg-red-600 hover:bg-red-700 text-white">
            <PhoneOff size={22} />
          </button>
          <button onClick={handleAccept} className="w-14 h-14 rounded-full bg-green-500 hover:bg-green-600 text-white">
