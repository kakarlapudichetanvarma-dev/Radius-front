...
export default function RegisterPage() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<RegisterRequest>({ resolver: zodResolver(registerSchema) });
    ...
    <div className="min-h-screen bg-white text-slate-900 flex overflow-hidden relative">
      ...
      >
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.2, ease: SMOOTH }} className="relative z-10">
          <h2 className="text-5xl font-extrabold leading-[1.1] tracking-tight mb-3 text-slate-900">
            Join the next<br />
            <span className="bg-gradient-to-r from-violet-600 to-fuchsia-600 bg-clip-text text-transparent">generation of chat.</span>
          </h2>
          <p className="text-slate-500 text-base leading-relaxed max-w-sm">
            Create your free account and connect with your team in seconds.
          </p>
        </motion.div>
          ...
          <div className="flex-1 rounded-2xl border border-violet-200 bg-white overflow-hidden shadow-xl shadow-violet-500/10 flex flex-col">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-semibold text-sm text-slate-900">Radius Team</p>
                  <p className="text-emerald-500 text-xs flex items-center gap-1.5">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block animate-pulse" />4 members online
                  </p>
                </div>
              </div>
      ...
      <div className="flex-1 flex items-center justify-center px-8 py-10 relative z-10">
        ...
        >
          ...
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight mb-1.5 text-slate-900">Create account</h1>
            <p className="text-slate-500 text-sm">Free forever. Join thousands of teams on Radius.</p>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
            {[
              { field: 'username', label: 'Username', type: 'text', placeholder: 'e.g. alexmorgan', error: errors.username },
              { field: 'email',    label: 'Email address', type: 'email', placeholder: 'you@example.com', error: errors.email },
              { field: 'phoneNumber', label: 'Phone number', type: 'tel', placeholder: '+1 (555) 000-0000', error: errors.phoneNumber },
            ].map(({ field, label, type, placeholder, error: fieldErr }) => (
              <div key={field} className="flex flex-col gap-1.5">
                ...
                {fieldErr && <p className="text-red-500 text-xs">{fieldErr.message as string}</p>}
              </div>
            ))}
            ...
            <button type="submit" disabled={loading}
              ...
              ) : (<>Create Free Account <span className="group-hover:translate-x-1 transition-transform duration-200">→</span></>)}
            </button>
          </form>

          <div className="flex items-center gap-4">
            <div className="flex-1 h-px bg-slate-200" />
            <span className="text-slate-400 text-xs uppercase tracking-widest">secure signup</span>
            <div className="flex-1 h-px bg-slate-200" />
          </div>
