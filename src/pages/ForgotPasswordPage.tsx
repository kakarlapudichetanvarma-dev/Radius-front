...
import { Mail, Shield, Zap, Lock, Users, Phone, Video, MoreVertical } from 'lucide-react';
...
const MESSAGES = [
  { msg: "Forgot my password 😅",        own: false, time: "11:01" },
  { msg: "No worries, reset it fast ⚡",  own: true,  time: "11:01" },
  { msg: "OTP sent to email 📧",          own: false, time: "11:02" },
  { msg: "Back in seconds! 🔓",           own: true,  time: "11:02" },
] as const;
...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); await forgotPassword(email); };
    ...
    <div className="min-h-screen bg-white text-slate-900 flex overflow-hidden relative">
      ...
      >
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.2, ease: SMOOTH }} className="relative z-10">
          <h2 className="text-5xl font-extrabold leading-[1.1] tracking-tight mb-3 text-slate-900">Locked out?<br /><span className="bg-gradient-to-r from-violet-600 to-fuchsia-600 bg-clip-text text-transparent">We've got you.</span></h2>
          <p className="text-slate-500 text-base leading-relaxed max-w-sm">Enter your email and we'll send a secure OTP to reset your password in seconds.</p>
        </motion.div>
          ...
          <div className="flex-1 rounded-2xl border border-violet-200 bg-white overflow-hidden shadow-xl shadow-violet-500/10 flex flex-col">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-white">
              <div className="flex items-center gap-3">
                ...
                <div><p className="font-semibold text-sm text-slate-900">Radius Support</p><p className="text-emerald-500 text-xs flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block animate-pulse" />always available</p></div>
              </div>
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 0.9 }} className="relative z-10 text-slate-400 text-xs">
          Reset link expires in 10 minutes · Check spam if not received
        </motion.p>
      </motion.div>
      ...
      <div className="flex-1 flex items-center justify-center px-8 py-10 relative z-10">
        ...
        >
          ...
          <div>
            <div className="w-14 h-14 rounded-2xl bg-violet-50 border border-violet-200 flex items-center justify-center mb-5">
              <Mail className="w-7 h-7 text-violet-600" />
            </div>
            <h1 className="text-3xl font-extrabold tracking-tight mb-1.5 text-slate-900">Forgot password?</h1>
            <p className="text-slate-500 text-sm leading-relaxed">No problem. Enter your email and we'll send you a reset code.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <button type="submit" disabled={loading}
              className="group w-full py-3.5 bg-violet-600 hover:bg-violet-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold rounded-xl text-sm transition-all duration-200 shadow-lg shadow-violet-500/30 hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2">
              {loading ? (
                <><svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" /></svg>Sending OTP...</>
              ) : (<>Send Reset Code <span className="group-hover:translate-x-1 transition-transform duration-200">→</span></>)}
            </button>
          </form>

          <div className="flex items-center gap-4"><div className="flex-1 h-px bg-slate-200" /><span className="text-slate-400 text-xs uppercase tracking-widest">secure reset</span><div className="flex-1 h-px bg-slate-200" /></div>
          ...
          <p className="text-center text-slate-500 text-sm">
            ...
            <Link to="/login" className="text-violet-600 font-semibold hover:text-violet-500 transition-colors duration-200">Sign in →</Link>
          </p>
        </motion.div>
      </div>
