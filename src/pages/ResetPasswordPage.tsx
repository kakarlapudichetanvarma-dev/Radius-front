...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => { e.preventDefault(); await resetPassword(formData); };
  ...
  const sharedLeft = (
    ...
    >
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.2, ease: SMOOTH }} className="relative z-10">
        <h2 className="text-5xl font-extrabold leading-[1.1] tracking-tight mb-3 text-slate-900">Set your<br /><span className="bg-gradient-to-r from-violet-600 to-fuchsia-600 bg-clip-text text-transparent">new password.</span></h2>
        <p className="text-slate-500 text-base leading-relaxed max-w-sm">Enter the OTP from your email and choose a strong new password.</p>
      </motion.div>
        ...
        <div className="flex-1 rounded-2xl border border-violet-200 bg-white overflow-hidden shadow-xl shadow-violet-500/10 flex flex-col">
          <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between bg-white">
            <div className="flex items-center gap-3">
              ...
              <div><p className="font-semibold text-sm text-slate-900">Radius Security</p><p className="text-emerald-500 text-xs flex items-center gap-1.5"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block animate-pulse" />reset in progress</p></div>
            </div>
      ...
      <div className="flex-1 flex items-center justify-center px-8 py-10 relative z-10">
        ...
        >
          ...
          <div>
            ...
            <h1 className="text-3xl font-extrabold tracking-tight mb-1.5 text-slate-900">Reset password</h1>
            <p className="text-slate-500 text-sm leading-relaxed">Enter the OTP from your email and set a new password.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            {[
              ...
              { name: 'otp',         label: 'OTP code',      type: 'text',     placeholder: 'Enter OTP from email' },
            ].map(({ name, label, type, placeholder }) => (
              <div key={name} className="flex flex-col gap-1.5">
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-widest">{label}</label>
                <input type={type} name={name} value={(formData as any)[name]} onChange={handleChange} required placeholder={placeholder}
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-widest">New password</label>
              <div className="relative">
                <input type={showPassword ? 'text' : 'password'} name="newPassword" value={formData.newPassword} onChange={handleChange} required placeholder="Min. 8 characters"
                ...
                <button type="button" onClick={() => setShowPassword(v => !v)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-violet-600 transition-colors duration-200">
          ...
          <p className="text-center text-slate-500 text-sm">
            <Link to="/login" className="text-violet-600 font-semibold hover:text-violet-500 transition-colors duration-200">← Back to Sign In</Link>
          </p>
        </motion.div>
      </div>
